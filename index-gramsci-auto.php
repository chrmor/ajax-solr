...
function addFacets($tagFacet, $placeHolder, $last, $minCount, $prefix) {
	...
	file_get_contents('http://localhost:8080/solr-gramsci-auto/collection1/select?q=*%3A*&start=1&wt=json&indent=true&facet=true&facet.query=*%3A*&facet.mincount=' . $minCount . '&facet.field=' . $tagFacet);
	//echo PHP_VERSION . "<br/>";
...
$time_start = microtime_float();
$conf = file_get_contents('gramsci-auto/js/conf_template.js');
$time_end = microtime_float();
//echo "JS file read in ".($time_end - $time_start)." milliseconds.<br/>";
//Get the content of the js config file...
$time_start = microtime_float();
$html = file_get_contents('gramsci-auto/html/html-template.html');
$time_end = microtime_float();
...
$time_start = microtime_float();
file_put_contents("gramsci-auto/js/demo.js",$conf);
$time_end = microtime_float();
//echo "JS updated in ".($time_end - $time_start)." milliseconds.<br/>";

//echo "Updating html index file...<br/>";
$time_start = microtime_float();
file_put_contents("index-gramsci-auto.html",$html);
$time_end = microtime_float();
